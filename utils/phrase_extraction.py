...
def chunk_generator_adapter(obj, chunk_size):
    ...
    while True:
        import sqlalchemy
        # 输入database connection object = conn.execute(query)
        if isinstance(obj, sqlalchemy.engine.result.ResultProxy):
            obj_adapter = list(obj.fetchmany(chunk_size))
        # 输入object = open(file_name, 'r', encoding='utf-8')
        elif isinstance(obj, _io.TextIOWrapper):
        ...
        if obj_adapter != None and obj_adapter != []:
            ...
            config.logger.info(
                '~~~ Time used for data processing: {} seconds'.format(sec_used))
            break
...
def extract_phrase(corpus,
    ...
    if isinstance(corpus, str):
        corpus_splits = [remove_irregular_chars(
            sent) for sent in sentence_split_by_punc(corpus)]
    ...
    word_info_scores = get_scores(
        corpus_splits, min_n, max_n, chunk_size, min_freq)
